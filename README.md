...
Before running `bayes_rest()`, you can use `bayes_stay_selection()` to
compare candidate probability distributions for staying time via WAIC
and choose the best-fitting one.

Supported distributions: `"exponential"`, `"gamma"`, `"weibull"`,
`"lognormal"`.
...
- A WAIC table comparing all candidate models (covariate subsets ×
  random effect options), with the best model marked.
...
Pass the data prepared above to `bayes_rest()`. Set `focal_area` to the
size of the camera’s focal area in **square metres** — this is the area
visible to the camera within which passes and staying time are measured.
...
`model = "RAD-REST"`. Now `formula_enter` specifies covariates on the
