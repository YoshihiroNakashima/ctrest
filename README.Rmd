...
-   R ≥ 4.1
-   [NIMBLE](https://r-nimble.org/) is used internally for
    Bayesian MCMC. On Windows,
...
```{r print-result}
print.ResultDensity(result_rest)
print.ResultDensity(result_rad)
```
...
print.ResultDensity(result_cov)
# One row per station
...
print.ResultDensity(result_allcomb)
...
print.ResultDensity(result_multi)
```
...
| `print.ResultStay()` | Print staying time model selection summary (`print()` also works) |
| `print.ResultDensity()` | Print density estimation summary (`print()` also works) |
| `print.ResultActivity()` | Print activity estimation summary without plot (`print()` also works) |
