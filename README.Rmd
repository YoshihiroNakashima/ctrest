...
```{r format-station-rad, eval = TRUE}
# RAD-REST
...
```{r plot-activity-curve, eval = FALSE}
library(ggplot2)
...
```{r station-effort-rad, eval = TRUE}
...
-   **`density`** (individuals per km²) — computed via the same formula
    as REST; units and interpretation are unchanged.
