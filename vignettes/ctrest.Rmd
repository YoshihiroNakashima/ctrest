...
station_effort_REST <- add_effort(
  ...
  font_size = 5                   # font size for visualization ticks
)
...
+------------------+-------------+--------------------------------------------------------------------------------------------------------+
| target_species   |             | 推定対象種（1種のみ指定可能）                                                                          |
|                  |             |                                                                                                        |
|                  |             | Species that targeted for staying time estimation (Only a single species can be specified.)            |
+------------------+-------------+--------------------------------------------------------------------------------------------------------+
...
+-----------------------+-------------+--------------------------------------------------------------------------------------------------------+
| randome_effect        |             | 滞在時間のランダム効果                                                                                 |
|                       |             |                                                                                                        |
|                       |             | A character string specifying a random effect on mean staying time.                                    |
+-----------------------+-------------+--------------------------------------------------------------------------------------------------------+
...
# library(nimble)
mdl <- bayes_rest(
  formula_stay = Stay ~ 1,        # Model formula (left-hand side is column name)
  formula_density = ~ 1 + x1,     # Formula for density (no left-hand side)
  station_effort_data = station_effort_REST,  # Return value from add_effort function
  stay_data = stay_data,          # Return value from format_stay function
  activity_data = activity_data,  # Return value from format_activity function
  activity_estimation = "kernel", # A Method for activity estimation
  bw_adj = 1.5,                   # Bandwidth for kernel density estimation
  stay_family = "lognormal",      # Distribution assumed for staying times
  focal_area = 1.96,              # Effective detection area
  cores = 2,                        
  iter = 3000,
  warmup = 1000,
  chains = 2,
  thin = 2,
  model = "REST",                 # REST/RAD-REST
  all_comb = FALSE,               # See above
  target_species = "SP01"
)
```
...
mdl$WAIC
mdl$summary_result
MCMCvis::MCMCtrace(object = mdl$samples,
                   params = c("density"),
                   pdf = FALSE)
```
...
mdl <- bayes_rest(
  formula_stay = Stay ~ 1,      # Model formula (left-hand side is column name)
  formula_density = ~ 1 + x1,   # Model formula for density (no left-hand side)
  station_effort_data = station_effort_RAD,  # Return value from add_effort function
  stay_data = stay_data,        # Return value from format_stay function
  activity_data = activity_data,# Return value from format_activity function
  bw_adj = 1.5,                 # Bandwidth for kernel density estimation
  stay_family = "lognormal",    # Distribution assumed for staying times
  ...
  chains = 2,
  thin = 2,
  model = "RAD-REST",           # REST/RAD-REST
  all_comb = FALSE,             # See the explanation for bayes_stay_selection
  target_species = "SP01"
)
```
